...
public class MenuItemController {
    ...
    @PostMapping
    public SimpleResponse saveMenu(@RequestBody @Valid MenuItemRequest menuItemRequest) {
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasAnyAuthority('CHEF','ADMIN')")
    public SimpleResponse updateMenu(@PathVariable Long id, @RequestBody @Valid MenuItemRequest menuItemRequest) {
    ...
    @GetMapping("/page")
    @PreAuthorize("permitAll()")
    public PaginationResponse paginationResponse(@RequestParam int currentPage, @RequestParam int size ){
        return menuItemService.getMenuItemPage(currentPage,size);
    }
